...
from api.deps import get_current_user, get_session_id
...
def _ingest_documents(documents, owner: str, document_id_override: str | None = None) -> tuple[int, list[str]]:
    ...
    for document in documents:
        ...
        # each Chunk's metadata FROM document.metadata).
...
@router.get("")
def get_summary(owner: str = Depends(get_current_user)):
    # Chunk counts summed from list_documents (already owner-scoped)
    # rather than store.count(kb), which has no owner filter - keeps
    # this summary consistent with the owner-scoped document list
    # right below it instead of mixing "your documents" with
    # "everyone's chunk totals".
    docs_by_kb = {kb: list_documents(kb, owner) for kb in KB_LABELS}
    return {
        "total_sources": sum(len(docs) for docs in docs_by_kb.values()),
        "total_chunks": sum(doc["chunk_count"] for docs in docs_by_kb.values() for doc in docs),
        "kbs": {kb: sum(doc["chunk_count"] for doc in docs) for kb, docs in docs_by_kb.items()},
    }


@router.get("/{kb}")
def list_sources(kb: str, session_id: str = Depends(get_session_id), owner: str = Depends(get_current_user)):
    indexed = {doc["id"]: doc["chunk_count"] for doc in list_documents(kb, owner)}
    pending_ids = {
...
def github_repositories(owner: str = Depends(get_current_user)):
    return list_repositories(owner)


def _run_file_ingest(kb: str, filename: str, suffix: str, content: bytes, *, session_id: str, owner: str, progress):
    ...
    try:
        ...
        _ingest_documents(documents, owner, document_id_override=filename)
        ...
        if kb == "video":
            for document in documents:
                _ingest_video_frames(document, tmp_path, owner)
        if kb in ("audio", "video"):
            for document in documents:
                _ingest_audio_clips(document, tmp_path, owner)
        add_pending(session_id, kb, {"document_id": filename, "label": filename, "owner": owner})
    finally:
...
async def upload_file(
    kb: str, file: UploadFile, session_id: str = Depends(get_session_id), owner: str = Depends(get_current_user)
):
    content = await file.read()
    suffix = Path(file.filename).suffix
    job_id = start_job(_run_file_ingest, kb, file.filename, suffix, content, session_id=session_id, owner=owner)
...
def _run_web_ingest(url: str, crawl: bool, max_pages: int | None, *, session_id: str, owner: str, progress):
    if crawl:
        ...
        _ingest_documents(documents, owner)
        for doc in documents:
            _ingest_images(doc, owner)
    ...
    _ingest_documents([document], owner)
    _ingest_images(document, owner)
    add_pending(session_id, "web", {"document_id": url, "label": url, "owner": owner})
    return {"pages_ingested": 1, "saved": False, "document_id": url}


@router.post("/web/ingest")
def ingest_web(
    body: WebIngestRequest, session_id: str = Depends(get_session_id), owner: str = Depends(get_current_user)
):
    job_id = start_job(
        _run_web_ingest, body.url, body.crawl, body.max_pages, session_id=session_id, owner=owner
    )
    return {"job_id": job_id}
...
def _run_github_ingest(repo_url: str, branch: str | None, *, session_id: str, owner: str, progress):
    progress({"message": f"Ingesting {repo_url}..."})
    ingest_github_repo(repo_url, branch=branch, owner=owner)
    parsed = repo_url.rstrip("/").removesuffix(".git")
    parts = parsed.split("/")
    owner_repo = "/".join(parts[-2:]) if len(parts) >= 2 else parsed
    add_pending(session_id, "github", {"repository": owner_repo, "label": owner_repo, "owner": owner})
    return {"repository": owner_repo}


@router.post("/github/ingest")
def ingest_github(
    body: GitHubIngestRequest, session_id: str = Depends(get_session_id), owner: str = Depends(get_current_user)
):
    job_id = start_job(_run_github_ingest, body.repo_url, body.branch, session_id=session_id, owner=owner)
    return {"job_id": job_id}
...
def _run_github_activity_ingest(
    ...
    *,
    ...
    add_pending(
        session_id, "github", {"repository": result["repository"], "label": result["repository"], "owner": owner}
    )
    return result
...
def ingest_github_activity_route(
    body: GitHubActivityIngestRequest,
...
@router.delete("/{kb}")
def delete_source(
    kb: str, doc_id: str, session_id: str = Depends(get_session_id), owner: str = Depends(get_current_user)
):
    if kb == "github":
        delete_repository(doc_id, owner)
        remove_pending(session_id, kb, "repository", doc_id)
    else:
        delete_document(doc_id, owner, kb=kb)
